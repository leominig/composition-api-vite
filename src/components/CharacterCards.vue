...
// un template REf se inicializza así :  const input = ref (null)
//onMounted 
...
function remove(character) {
  data.results=data.results.filter((entry) => entry.name!==entry.name)
}
    ...
    <div class="m-auto container flex flex-wrap mt-10">
      ...
      >
        <div class="card-inner">
          ...
          <slot name="secondrow" :character="character" :remove="remove"></slot>
        </div>
      </div>
      <div>
